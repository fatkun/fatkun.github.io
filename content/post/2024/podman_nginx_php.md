---
title: "使用podman部署nginx和php"
date: 2024-08-30T14:22:01+08:00
tags: [linux]
categories: [linux]
author: "fatkun"
draft: false
...
podman不需要常驻进程，能够节省一些内存，但是启动服务需要放入systemd。

这次目标是部署nginx和php-fpm，一般使用默认配置，非生产环境。

# 步骤

我们需要启动php-fpm，然后nginx通过端口和php-fpm通信，这里使用pod的方式进行。

```bash
# 需要指定端口
podman pod create --name nginxpod -p 80:80 -p 443:443
```

## php-fpm部署

创建conf目录，创建php.ini和www.conf
...
;ffi.preload=
root@vps2:~/docker/php/conf# ^C
root@vps2:~/docker/php/conf# ls
php.ini  www.conf
root@vps2:~/docker/php/conf# ll
total 96
-rw-r--r-- 1 root root 73412 Jul  3 22:29 php.ini
-rw-r--r-- 1 root root 22096 Dec 19  2023 www.conf
root@vps2:~/docker/php/conf# cat www.conf 
...
root@vps2:~/docker/php/conf# ^C
root@vps2:~/docker/php/conf# ll^C
root@vps2:~/docker/php/conf# ll
total 96
-rw-r--r-- 1 root root 73412 Jul  3 22:29 php.ini
-rw-r--r-- 1 root root 22096 Dec 19  2023 www.conf
root@vps2:~/docker/php/conf# cd ..
root@vps2:~/docker/php# ls
conf  log  start_php.sh
root@vps2:~/docker/php# ll
total 12
drwxr-xr-x 2 root root 4096 Jul  3 22:29 conf
drwxr-xr-x 2 root root 4096 Dec 24  2023 log
-rw-r--r-- 1 root root  228 Dec 24  2023 start_php.sh
root@vps2:~/docker/php# cat start_php.sh 
podman run --name php-fpm --pod nginxpod \
-v /root/docker/nginx/www:/var/www/html \
-v /root/docker/php/conf/www.conf:/usr/local/etc/php-fpm.d/www.conf \
-v /root/docker/php/conf/php.ini:/usr/local/etc/php/php.ini \
-d php:fpm
root@vps2:~/docker/php# ls
conf  log  start_php.sh
root@vps2:~/docker/php# ls log
root@vps2:~/docker/php# podman cd ^C
root@vps2:~/docker/php# cd conf/
root@vps2:~/docker/php/conf# ls
php.ini  www.conf
root@vps2:~/docker/php/conf# ll
total 96
-rw-r--r-- 1 root root 73412 Jul  3 22:29 php.ini
-rw-r--r-- 1 root root 22096 Dec 19  2023 www.conf
root@vps2:~/docker/php/conf# cat php.ini 
[PHP]
...
server {
    ...
    # 开启 HTTP/2
    http2 on;
    server_name  fatkun2.ddnsfree.com;
...
podman由于没有常驻进程，启动需要依赖systemd，执行命令会生成3个service(container-nginx.service  container-php-fpm.service  pod-nginxpod.service)文件

```bash
podman generate systemd --files --name  nginxpod
```

把service拷贝到systemd目录

```bash
cp *.service /etc/systemd/system
# 开启服务
systemctl enable pod-nginxpod
```
